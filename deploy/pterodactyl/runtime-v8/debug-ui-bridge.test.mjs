...
test('retired scope-review diagnostics stay inert after the correctness gate was removed', () => {
  ...
  debug = liveAgentDebugEvent('planning.scope_review_failed', {
    ...
    reason: 'retired event must not repopulate diagnostics',
  }, debug)
  assert.equal(debug.decision_scope_review, undefined)
  assert.equal(debug.decision_scope_review_failure_stage, undefined)
  assert.equal(debug.decision_scope_review_failure_reason, undefined)
