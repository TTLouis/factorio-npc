...
test('in-game task board snapshot is a projection of canonical durable state', () => {
  ...
  // Jev scope review was retired from the commit path with its telemetry.
  assert.equal('decision_scope_review' in snapshot.debug, false)
  ...
  // Retired with the Jev semantic admission gate.
  assert.equal('step_relation' in snapshot.debug, false)
  assert.equal('step_checkpoint_boundary' in snapshot.debug, false)
  assert.equal('step_admission_alignment' in snapshot.debug, false)
