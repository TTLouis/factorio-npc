...
A1  deterministic exact-layout fixture
    runtime receives a known-good exact construction/configuration target
        |
        v
    proves harness / engine mechanics

A2  guided AI arrangement
    machine set + topology + bounded site are supplied
        |
        v
    AI chooses exact arrangement and executes

A3  autonomous AI production objective
    only the production objective / allowed scope is supplied
        |
        v
    AI chooses route / topology / arrangement / execution
```
