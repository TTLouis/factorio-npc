...
- compress authoritative world/runtime evidence into bounded semantic working memory;
...
authoritative runtime snapshot
   |
   v
JEV context reduction / observation selection
   |------------------------------+
   | missing useful fact          |
   v                              |
...
schema + identity + preflight + safety
   |
   v
FACTORIO / AUTORIO
   |
...
A future compact decision may resemble:
...
```json
{
  ...
  "reason": "Need current bootstrap capability before waking the planner."
}
```

All observations remain deterministic reads. Jev does not manufacture their values.

## 6. Semantic working memory

Jev should compress authoritative facts into a bounded semantic state for the Main LLM.

Example:

```text
Goal: automate iron and copper plates

Iron:
- patch located
- burner drill available
- no furnace placed
...
Fuel:
- coal available nearby
- held fuel sufficient for bootstrap

Stone:
- insufficient for next furnace
...
This summary is not world truth. It is a cache/compression product derived from referenced authoritative facts. The Main LLM should still receive enough provenance or raw facts to verify any material claim it relies on.
...
- add bounded semantic working-memory compression;
