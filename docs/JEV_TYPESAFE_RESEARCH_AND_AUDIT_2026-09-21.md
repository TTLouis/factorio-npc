...
The post-step System One request now adds four bounded typed features over authoritative
runtime state:

- `state_bottleneck` Choice over ten coarse semantic bottleneck categories;
- `state_readiness` Score over five grounded-readiness levels;
- `state_risk` Score over four consequence/reversibility levels;
- `state_evidence_conflict` Noul.

The parser accepts only those bounded primitives. Provenance is generated by deterministic
code from the state sections actually supplied to Jev, and code renders a fixed
`[JEV_TYPED_STATE]` block into the post-step Main-LLM continuation. Invalid or absent
typed-state answers render no block.

The block explicitly has no world-truth, completion, plan, operation-admission, or
user-authority role. The M8 questions share the existing post-step provider request
instead of creating a second Jev round.

Exit condition status:

- bounded typed distillation, deterministic provenance, fixed rendering, malformed-output
  fallback, and live post-step injection are covered by focused tests;
- measuring context/token reduction remains later E2E work.
