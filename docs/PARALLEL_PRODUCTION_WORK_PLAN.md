...
| # | Item | Detail | Model |
|---|---|---|---|
| 1.1 | Placement footprint | P0 section above: shared footprint helper, `place_entity` reasons, placement by relation, candidate footprints, `plan_placement` fixes, engine lane for the self-feeding drill pair. **In progress in another session** (`032d02e`..`63fec8a`, pushed 2026-09-25): footprint helper, size-aware candidates and `plan_placement`, refusal geometry in receipts, relational-placement guidance, a production-lane cell. At `63fec8a` two of its unit tests fail (`basic_operation_runtime.placement.test.ts` "rejects the live burner-drill half-tile center…", `placement_geometry.test.ts` "swaps non-square footprint grid rules…"). Finish: make those green, confirm the engine lane passes, then tick the P0 list | Opus |
